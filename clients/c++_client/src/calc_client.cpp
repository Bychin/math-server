...
int main(int argc, char **argv) {
    ...
    // Послать на сервер запрос типа C для вычислений.
    json calc_query = json({
        {"func", "mul"},
        {"data", "my_data"}
    });
    send_string(socket_desc, build_request("C", calc_query));

    // Пытаемся получить ответ.
    if (recv(socket_desc, server_message, MSG_SIZE, 0) < 0) {
        std::cout << "[-] Error! Cannot get answer";
        return -7;
    }
    std::cout << "[+] Answer was recieved: " << server_message;
    memset(server_message, 0, MSG_SIZE); // Очистить массив для дальнейшего использования

    // Послать на сервер другой запрос типа C для вычислений.
    calc_query = json({
        {"func", "div"},
        {"data", "div_data"}
    });
